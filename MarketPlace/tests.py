...
class GetProductCategoriesTestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        user = User.objects.create_user(email="test@domain.com", password="password")
        self.marketplace = MarketPlace.objects.create(
            name="E-commerce", cover_image="path/to/image.extension"
        )
        vendor = StoreVendor.objects.create(user=user, email=user.email)
    ...
    def test_get_all_categories(self):
        ProductCategory.objects.create(
            **{"marketplace": self.marketplace, "name": "Food & Health"}
        )
        url = reverse(
            "MarketPlace:marketplace-product-categories-list", kwargs={"pk": 1}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(hasattr(response, "json"))
    ...
    def test_get_all_categories_with_invalid_pk(self):
        url = reverse(
            "MarketPlace:marketplace-product-categories-list", kwargs={"pk": 0}
        )
        response = self.client.get(url)
    ...
    def test_get_popular_categories(self):
        ...
        url = reverse(
            "MarketPlace:marketplace-popular-product-categories-list", kwargs={"pk": 1}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(hasattr(response, "json"))
    ...
    def test_get_popular_categories_with_invalid_pk(self):
        url = reverse(
            "MarketPlace:marketplace-popular-product-categories-list", kwargs={"pk": 0}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertTrue(hasattr(response, "json"))
...
class GetCartViewTestCase(TestCase):
    def setUp(self):
        ...
        vendor = StoreVendor.objects.create(user=self.user, email=self.user.email)
        store = Store.objects.create(
    ...
    def test_get_user_cart_when_not_empty(self):
        ...
        self.assertTupleEqual(
            ...
            (
                1,
                False,
                round(self.product.discounted_price * 5, 2),
...
class OrderModelTestCase(TestCase):
    ...
    def test_order_creation(self):
        order = Order.objects.create(
            buyer=self.user, product=self.product, quantity=2, status="shipped"
        )
